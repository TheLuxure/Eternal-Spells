...
public class LargeMageWoodForestBiome extends EternalSpellsModElements.ModElement {
	@ObjectHolder("eternal_spells:large_mage_wood_forest")
	public static final CustomBiome biome = null;
	public LargeMageWoodForestBiome(EternalSpellsModElements instance) {
		super(instance, 44);
	}
	...
	static class CustomBiome extends Biome {
		public CustomBiome() {
			super(new Biome.Builder().downfall(0.5f).depth(0.1f).scale(0.2f).temperature(0.5f).precipitation(Biome.RainType.RAIN)
					.category(Biome.Category.FOREST).waterColor(4159204).waterFogColor(329011).parent("eternal_spells:magewood_forest")
					.surfaceBuilder(SurfaceBuilder.DEFAULT, new SurfaceBuilderConfig(Blocks.COARSE_DIRT.getDefaultState(),
							Blocks.STONE.getDefaultState(), Blocks.STONE.getDefaultState())));
			setRegistryName("large_mage_wood_forest");
			...
			addFeature(GenerationStage.Decoration.VEGETAL_DECORATION,
					new CustomTreeFeature()
							.withConfiguration((new BaseTreeFeatureConfig.Builder(new SimpleBlockStateProvider(MagewoodBlock.block.getDefaultState()),
									new SimpleBlockStateProvider(MageleavesBlock.block.getDefaultState()))).baseHeight(14)
											.setSapling((net.minecraftforge.common.IPlantable) Blocks.JUNGLE_SAPLING).build())
							.withPlacement(Placement.COUNT_EXTRA_HEIGHTMAP.configure(new AtSurfaceWithExtraConfig(3, 0.1F, 1))));
			this.addSpawn(EntityClassification.MONSTER, new Biome.SpawnListEntry(NecromancerEntity.entity, 60, 8, 12));
			this.addSpawn(EntityClassification.MONSTER, new Biome.SpawnListEntry(EntityType.WITHER_SKELETON, 35, 8, 12));
			this.addSpawn(EntityClassification.MONSTER, new Biome.SpawnListEntry(EntityType.PHANTOM, 5, 8, 12));
		}
	}
	...
	static class CustomTreeFeature extends AbstractTreeFeature<BaseTreeFeatureConfig> {
		...
		private boolean canGrowInto(Block blockType) {
			return blockType.getDefaultState().getMaterial() == Material.AIR || blockType == MagewoodBlock.block.getDefaultState().getBlock()
					|| blockType == MageleavesBlock.block.getDefaultState().getBlock() || blockType == Blocks.COARSE_DIRT.getDefaultState().getBlock()
